...
public class MayinTarlasi implements MouseListener {
        ...
        for (int row = 0; row < board.length; row++) {
            for (int col = 0; col < board[0].length; col++) {
                if (board[row][col].isMine()) {
                    board[row][col].setIcon(new ImageIcon("minee.png"));
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getButton() == 1) {
            if (b.isMine()) {
                JOptionPane.showMessageDialog(frame, "Mayın Patladı!");
                print();
            } else {
            ...
            if (!b.isFlag()) {
                b.setIcon(new ImageIcon("danger.png"));
                b.setFlag(true);
            } else {
                b.setIcon(null);
                b.setFlag(false);
